@extends('layouts.admin')
@section('content')
@can('sent_instruction_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route("admin.sent-instructions.create") }}">
                {{ trans('global.add') }} {{ trans('cruds.sentInstruction.title_singular') }}
            </a>
        </div>
    </div>
@endcan
<div class="card">
    <div class="card-header">
        {{ trans('cruds.sentInstruction.title_singular') }} {{ trans('global.list') }}
    </div>

    <div class="card-body">
        <table class=" table table-bordered table-striped table-hover ajaxTable datatable datatable-SentInstruction">
            <thead>
                <tr>
                    <th width="10">

                    </th>
                    <th>
                        {{ trans('cruds.sentInstruction.fields.id') }}
                    </th>
                    <th>
                        {{ trans('cruds.sentInstruction.fields.url') }}
                    </th>
                    <th>
                        {{ trans('cruds.sentInstruction.fields.validation_date') }}
                    </th>
                    <th>
                        {{ trans('cruds.sentInstruction.fields.user') }}
                    </th>
                    <th>
                        {{ trans('cruds.sentInstruction.fields.status') }}
                    </th>
                    <th>
                        {{ trans('cruds.sentInstruction.fields.workers_list') }}
                    </th>
                    <th>
                        {{ trans('cruds.sentInstruction.fields.instruction') }}
                    </th>
                    <th>
                        {{ trans('cruds.sentInstruction.fields.worker') }}
                    </th>
                    <th>
                        &nbsp;
                    </th>
                </tr>
            </thead>
        </table>
    </div>
</div>



@endsection
@section('scripts')
@parent
<script>
    $(function () {
  let dtButtons = $.extend(true, [], $.fn.dataTable.defaults.buttons)
@can('sent_instruction_delete')
  let deleteButtonTrans = '{{ trans('global.datatables.delete') }}';
  let deleteButton = {
    text: deleteButtonTrans,
    url: "{{ route('admin.sent-instructions.massDestroy') }}",
    className: 'btn-danger',
    action: function (e, dt, node, config) {
      var ids = $.map(dt.rows({ selected: true }).data(), function (entry) {
          return entry.id
      });

      if (ids.length === 0) {
        alert('{{ trans('global.datatables.zero_selected') }}')

        return
      }

      if (confirm('{{ trans('global.areYouSure') }}')) {
        $.ajax({
          headers: {'x-csrf-token': _token},
          method: 'POST',
          url: config.url,
          data: { ids: ids, _method: 'DELETE' }})
          .done(function () { location.reload() })
      }
    }
  }
  dtButtons.push(deleteButton)
@endcan

  let dtOverrideGlobals = {
    buttons: dtButtons,
    processing: true,
    serverSide: true,
    retrieve: true,
    aaSorting: [],
    ajax: "{{ route('admin.sent-instructions.index') }}",
    columns: [
      { data: 'placeholder', name: 'placeholder' },
{ data: 'id', name: 'id' },
{ data: 'url', name: 'url' },
{ data: 'validation_date', name: 'validation_date' },
{ data: 'user_name', name: 'user.name' },
{ data: 'status_name', name: 'status.name' },
{ data: 'workers_list_name', name: 'workers_list.name' },
{ data: 'instruction_name', name: 'instruction.name' },
{ data: 'worker', name: 'workers.name' },
{ data: 'actions', name: '{{ trans('global.actions') }}' }
    ],
    order: [[ 1, 'desc' ]],
    pageLength: 100,
  };
  $('.datatable-SentInstruction').DataTable(dtOverrideGlobals);
    $('a[data-toggle="tab"]').on('shown.bs.tab', function(e){
        $($.fn.dataTable.tables(true)).DataTable()
            .columns.adjust();
    });
});

</script>
@endsection