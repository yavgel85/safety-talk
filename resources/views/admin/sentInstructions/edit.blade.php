...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.sentInstruction.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.sent-instructions.update", [$sentInstruction->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="url">{{ trans('cruds.sentInstruction.fields.url') }}</label>
                <input class="form-control {{ $errors->has('url') ? 'is-invalid' : '' }}" type="text" name="url" id="url" value="{{ old('url', $sentInstruction->url) }}">
                @if($errors->has('url'))
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('validation_date') ? 'is-invalid' : '' }}" type="text" name="validation_date" id="validation_date" value="{{ old('validation_date', $sentInstruction->validation_date) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id">
                    @foreach($users as $id => $user)
                        <option value="{{ $id }}" {{ ($sentInstruction->user ? $sentInstruction->user->id : old('user_id')) == $id ? 'selected' : '' }}>{{ $user }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status_id" id="status_id">
                    @foreach($statuses as $id => $status)
                        <option value="{{ $id }}" {{ ($sentInstruction->status ? $sentInstruction->status->id : old('status_id')) == $id ? 'selected' : '' }}>{{ $status }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('workers_list') ? 'is-invalid' : '' }}" name="workers_list_id" id="workers_list_id">
                    @foreach($workers_lists as $id => $workers_list)
                        <option value="{{ $id }}" {{ ($sentInstruction->workers_list ? $sentInstruction->workers_list->id : old('workers_list_id')) == $id ? 'selected' : '' }}>{{ $workers_list }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('instruction') ? 'is-invalid' : '' }}" name="instruction_id" id="instruction_id">
                    @foreach($instructions as $id => $instruction)
                        <option value="{{ $id }}" {{ ($sentInstruction->instruction ? $sentInstruction->instruction->id : old('instruction_id')) == $id ? 'selected' : '' }}>{{ $instruction }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('workers') ? 'is-invalid' : '' }}" name="workers[]" id="workers" multiple>
                    @foreach($workers as $id => $worker)
                        <option value="{{ $id }}" {{ (in_array($id, old('workers', [])) || $sentInstruction->workers->contains($id)) ? 'selected' : '' }}>{{ $worker }}</option>
                    @endforeach
                </select>
