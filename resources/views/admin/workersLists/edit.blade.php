...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.workersList.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.workers-lists.update", [$workersList->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.workersList.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $workersList->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_listed') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_listed" id="is_listed" value="1" {{ $workersList->is_listed || old('is_listed', 0) === 1 ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('workers') ? 'is-invalid' : '' }}" name="workers[]" id="workers" multiple>
                    @foreach($workers as $id => $worker)
                        <option value="{{ $id }}" {{ (in_array($id, old('workers', [])) || $workersList->workers->contains($id)) ? 'selected' : '' }}>{{ $worker }}</option>
                    @endforeach
                </select>
